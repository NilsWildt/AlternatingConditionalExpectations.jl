...
function mymean(x::AbstractArray{Float64})::Float64
    @tullio m = x[i]
    return m / length(x)
end
...
function 𝔼_conditional!(out, Y, X, myace::ACEsim, sindx, bindx)
    ...
    # Y = Y[sindx]
    # @infiltrate # @timeit "Smoothing"
    do_smoothing!(out, X[sindx], Y[sindx], myace.smoother)
    out .= @view out[bindx]
    # Y = Y[bindx]
    # X = X[bindx]
end
...
function stoch_normalize!(out, X::Array{Float64})::Array{Float64}
    tmpmean = mymean(X)
    out .= (X .- tmpmean) / std(X; corrected=true, mean=tmpmean)
end


function stoch_normalize!(X::Array{Float64})::Array{Float64}
    tmpmean = mymean(X)
    X = (X .- tmpmean) / std(X; corrected=true, mean=tmpmean)
end

function sum_wo(x, i::Int)
    return sum(x[:, 1:i-1], dims=2) .+ sum(x[:, i+1:end], dims=2)
end

function sum_wo_theta!(out, θ, x, i::Int)
    out .= θ .- @views sum(x[:, 1:i-1], dims=2) .+ sum(x[:, i+1:end], dims=2)
end
    ...
    while (abs(e_old - e_new) > myace.errorbound) && i <= myace.itermax_outer #
        ...
        while (abs(e_old - e_new) > myace.errorbound) && j <= myace.itermax_inner#
            ...
            @inbounds for k in 1:m_parameter
                # θ_without_Φ_k = Θ_y .- sum_wo(Φ_x, k)
                sum_wo_theta!(θ_without_Φ_k, Θ_y, Φ_x, k)
                𝔼_conditional!(view(Φ_x, :, k), θ_without_Φ_k, X[:, k], myace, sIx[:, k], bsIx[:, k]) # E_y(...)
                # Φ_x[:,k] =  𝔼_conditional( θ_without_Φ_k, X[:,k], myace, sIx[:,k], bsIx[:,k]) # E_y(...)
                Φ_x[:, k] = Φ_x[:, k] .- mean(Φ_x[:, k])
            end
            e_new = ε²(Θ_y, Φ_x)
        ...
        Θ_y = stoch_normalize(Θ_y) #  @timeit "Normalize2"
        # Θ_y .= Θ_y
        i += 1
        e_new = ε²(Θ_y, Φ_x) # @timeit "errorcalc" 
...
# Plot recipie.
@recipe function f(bf::ACEres; transform=false, full=true, dpi=500, plotsize=2.0 .* (1200, 450))
