...
begin
	...
	# Data on x y
	figure, axs, plot = Makie.meshscatter(x,nn.-3.0, y, figure = (resolution = (850,900),label="1. Datapoints", legend=:left, ),markersize = 0.1,shading = true,color=datacolor, marker = sphere,perspectiveness=0,axis = ( ;xlabel = "X", zlabel = "Y or Θ(y)",ylabel="Φ(x)", title = "ACE Visualization: y=exp(sin(x))", type = Axis3,aspects=:data,viewmode=:fitzoom, azimuth = 1*pi,protrusion= (0, 0, 0, 100),elevation=0.5), transparency = true,colormap = reverse(to_colormap(:viridis)))
	# 
	#  
	...
	band!(lower, upper,transparency=true,shading=true, color=planecolor, ssao=true,alpha=0.3)	
	...
	# Rueckwand data
		Makie.meshscatter!(nn.+7,Φ(x)|>normalize_mean,θ(y)|>normalize, markersize = 0.1,  transparency = true,color = RGBA((242, 100, 48,255)./255...), shading = true, marker = sphere)	
		
	
		Makie.meshscatter!(nn.+7,Φ(x)|>normalize_mean, (y|>normalize ).+3.0, markersize = 0.1, transparency = true,color =RGBA((0, 157, 220,150)./255...), shading = true, marker = sphere)	
...
manifest_format = "2.0"
project_hash = "61ef6864a378572f531d4ebb10ac6a41da445927"
