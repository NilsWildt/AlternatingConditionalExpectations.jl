...
# 	$(@bind α PlutoUI.Slider(0:0.05:1,default=0.5,show_value=true))
...
# ╔═╡ 6a089d38-2b1f-468c-86fc-5891b1bf46c8
 δ = 0.1
...
# @. θ(y) = α*(log(y))+(1.0-α)*asin(log(y));

# ╔═╡ 63a930e4-f3e1-40c6-9e04-5fab4915a2ed
@.function θ(y) 
	return  α*(log(y))+(1.0-α)*asin(log(y));
end

# ╔═╡ 7f82fd49-e0fb-403c-b6fc-b2712de53f63
begin
			GLMakie.activate!()
...
# ╔═╡ c3301264-02c6-44b0-a9a4-ac2881d40b24
Makie.scatter(x,y)
...
begin
	...
	# Marker
	sphere = Sphere(Point3f(0,0,0), 0.5f0)
		
	# Data on x y
	figure, axs, plot = Makie.meshscatter(x,nn.-3.0, y, figure = (resolution = (850,900),label="1. Datapoints", legend=:left, ),markersize = 0.1,shading = true,color=datacolor, marker = sphere,perspectiveness=0,axis = ( ;xlabel = "X", zlabel = "Y or Θ(y)",ylabel="Φ(x)", title = "ACE Visualization: y=exp(sin(x))", type = Axis3,viewmode=:fitzoom, azimuth = 1*pi,elevation=0.5), transparency = true,colormap = reverse(to_colormap(:viridis)))
	# 
	#  
		
		
	Makie.meshscatter!(x |>normalize_mean,nn.-2,(y |>normalize).+3, markersize = 0.1, transparency = true,color = RGBA((0, 157, 220,150)./255...), shading = true,alpha=0.3, marker = sphere)	
	
		
	Makie.meshscatter!(x |>normalize_mean,nn.-1.5,y |> θ |>normalize, markersize = 0.1,  transparency = true,color = RGBA((242, 100, 48,255)./255...), shading = true,alpha=0.3, marker = sphere)	
	
		
	# # Smooth on xy
	# Makie.meshscatter!(z,nn1,s, markersize = 0.1, transparency = true,colormap = reverse(to_colormap(:viridis)), shading = true,color=smoothcolor, marker = sphere)
	...
	lower = [Point3f(z[i],z[i] |> Φ, -2.0) for i in 1:length(z)]
	upper = [Point3f(z[i],z[i] |> Φ, 2.0) for i in 1:length(z)]
	band!(lower, upper,transparency=false,shading=false, color=(:blue, 0.1), ssao=false,alpha=1.0)	
	
	@. zzf(z) = exp(sin(z))	
		scene = Scene(show_axis = false)
	
	Makie.meshscatter!(x, x|>Φ |>normalize_mean, y|> θ|>normalize, markersize = 0.1,transparency = true,color = RGBA((242, 100, 48,255)./255...), shading = true,alpha=0.3, marker = sphere)	
	...
	# Rueckwand data
		Makie.meshscatter!(nn.+7,Φ(x)|>normalize_mean,θ(y)|>normalize, markersize = 0.1,  transparency = true,color = RGBA((242, 100, 48,255)./255...), shading = false, marker = sphere)	
	
		Makie.meshscatter!(nn.+7,Φ(x)|>normalize_mean, (y|>normalize ).+3.0, markersize = 0.1, transparency = true,color =RGBA((0, 157, 220,150)./255...), shading = false, marker = sphere)	
...
manifest_format = "2.0"
project_hash = "61be00a1eb50d1d468a48802ef4a97305f6a9d21"
