...
@testitem "run converges with LASb" begin
    ...
    res = AlternatingConditionalExpectations.ace_run(ACEsim(X, Y, LASb(15)))
    @test res isa ACEres
    @test res.ρ < 0.05
    @test res.r²[1] > 0.9
...
@testitem "run works with all smoother types" begin
    ...
    for sm in smoothers
        res = AlternatingConditionalExpectations.ace_run(ACEsim(X, Y, sm))
...
@testitem "run works with a vector of smoothers" begin
    X, Y = generate_bivariate_data(Float64, 200, 0.1, 1.0, 42)
    res = AlternatingConditionalExpectations.ace_run(ACEsim(X, Y, [LASb(10), LASb(12)]))
    @test all(isfinite, res.Φ_x)
    @test res.r²[1] > 0.8
end
...
@testitem "run on multivariate data" begin
    ...
    res = AlternatingConditionalExpectations.ace_run(ACEsim(X, Y, LLSSb(25)))
    @test res isa ACEres
    @test res.ρ < 0.1
    @test length(res.r²) == 2
...
@testitem "error metrics" begin
    ...
    yhat = y .+ 0.01
    @test AlternatingConditionalExpectations.RMSE(y, yhat) ≈ 0.01 atol = 1e-12
    @test AlternatingConditionalExpectations.MAE(y, yhat) ≈ 0.01 atol = 1e-12
    @test AlternatingConditionalExpectations.pErr(y, yhat) ≈ 0.0001 atol = 1e-12
    @test AlternatingConditionalExpectations.myErr(y, yhat) ≈ 0.01 atol = 1e-12
    @test isfinite(AlternatingConditionalExpectations.AARD(y, yhat))
    @test isfinite(AlternatingConditionalExpectations.UFV(y, yhat))
    @test isfinite(AlternatingConditionalExpectations.nMAE(y, yhat))
end
